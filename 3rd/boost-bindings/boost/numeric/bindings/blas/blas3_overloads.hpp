...
namespace boost { namespace numeric { namespace bindings { namespace blas { namespace detail {
  ...
  //
  inline
  void symm( char side, char uplo, const int& m, const int&n, const float& alpha, const float* a_ptr,
             const int lda, const float* b_ptr, const int& ldb, const float& beta, const float* c_ptr,
			 const int& ldc )
  {
     BLAS_SSYMM( &side, &uplo, &m, &n, &alpha, a_ptr, &lda, b_ptr, &ldb, &beta, c_ptr, &ldc );
  }

  inline
  void symm( char side, char uplo, const int& m, const int&n, const double& alpha, const double* a_ptr,
             const int lda, const double* b_ptr, const int& ldb, const double& beta, const double* c_ptr,
			 const int& ldc )
  {
     BLAS_DSYMM( &side, &uplo, &m, &n, &alpha, a_ptr, &lda, b_ptr, &ldb, &beta, c_ptr, &ldc );
  }

  inline
  void symm( char side, char uplo, const int& m, const int&n, const complex_f& alpha, const complex_f* a_ptr,
             const int lda, const complex_f* b_ptr, const int& ldb, const complex_f& beta, const complex_f* c_ptr,
			 const int& ldc )
  {
     BLAS_CSYMM( &side, &uplo, &m, &n, complex_ptr( &alpha ), complex_ptr( a_ptr ), &lda, 
		complex_ptr( b_ptr ), &ldb, complex_ptr( &beta ), complex_ptr( c_ptr ), &ldc );
  }

  inline
  void symm( char side, char uplo, const int& m, const int&n, const complex_d& alpha, const complex_d* a_ptr,
             const int lda, const complex_d* b_ptr, const int& ldb, const complex_d& beta, const complex_d* c_ptr,
			 const int& ldc )
  {
     BLAS_ZSYMM( &side, &uplo, &m, &n, complex_ptr( &alpha ), complex_ptr( a_ptr ), &lda, 
		complex_ptr( b_ptr ), &ldb, complex_ptr( &beta ), complex_ptr( c_ptr ), &ldc );
  }
