...
namespace boost { namespace numeric { namespace bindings { namespace blas {
  ...
  {
    const int m = traits::matrix_size1( c );
    const int n = traits::matrix_size2( c );
	assert( side == 'R' || side == 'L' || side == 'r' || side == 'l' );
    assert( !( side == 'L' || side == 'l' ) || 
		( m == traits::matrix_size1( a ) && n == traits::matrix_size2( b ) && traits::matrix_size2( a ) == traits::matrix_size1( b ) ) ) ;
    assert( !( side == 'R' || side == 'r' ) || 
		( m == traits::matrix_size1( b ) && n == traits::matrix_size2( a ) && traits::matrix_size2( b ) == traits::matrix_size1( a ) ) ) ;
    const int lda = traits::leading_dimension( a );
    const int ldb = traits::leading_dimension( b );
    const int ldc = traits::leading_dimension( c );
    ...
    detail::symm( side, uplo, m, n, alpha, a_ptr, lda, b_ptr, ldb, beta, c_ptr, ldc ) ;
  }
  ...
  void symm( const char side, const char uplo, const matrix_type_a &a, const matrix_type_b &b, matrix_type_c &c )
  {
    typedef typename traits::matrix_traits<matrix_type_c>::value_type val_t; 
    symm( side, uplo, (val_t) 1, a, b, (val_t) 0, c ) ;
  }
