...
namespace boost { namespace numeric { namespace bindings { 
  ...
  namespace lapack {
    ...
    /*
     * potri() computes the inverse from a cholesky factorization computed by potrf
     */

    namespace detail {

      inline 
      void potri (char const uplo, int const n, 
                  float* a, int const lda, int* info) 
      {
      ...
      template <typename SymmMatrA> 
      inline
      int potri (char const uplo, SymmMatrA& a) {
        int const n = traits::matrix_size1 (a);
        assert (n == traits::matrix_size2 (a));
        int info; 
        potri (uplo, n, traits::matrix_storage (a), 
               traits::leading_dimension (a), &info);
        return info; 
      }
