...
#include <utAlgorithm/FundamentalMatrix.h>
...
void TestFundamentalMatrix()
{
	for( int j=0; j<100; j++ )
	{
		...
		Math::Matrix< double, 3, 3 > F = Algorithm::fundamentalMatrixFromPoses( CamPose1, CamPose2, I, I );
		...
		Math::Matrix< double, 3, 3 > FTest = Algorithm::getFundamentalMatrix( fromPoints, toPoints );
