...
#include <utCalibration/HandEyeDataSelection.h>
...
void TestDataSelection( const std::size_t n_runs, const T epsilon )
{
	...
	for ( std::size_t iRun = 0; iRun < n_runs; iRun++ )
	{
		...
		Ubitrack::Calibration::select_6DPoses( leftFrame, rightFrame, selection, selectedPosesLeft, selectedPosesRight );
		// Ubitrack::Calibration::select_6DPoses( leftFrame, rightFrame, selection, std::back_inserter( selectedPosesLeft ), std::back_inserter( selectedPosesRight ) );
