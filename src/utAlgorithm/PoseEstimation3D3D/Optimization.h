...
public:
	...
	void jacobian( const VT2& input, MT& J ) const
	{
		...
		for ( InputIterator it ( m_iBegin ); it != m_iEnd; ++i, ++it )
		{
			...
			const VType t2 = qx*y*2.0;
			const VType t3 = qw*z*2.0;
			const VType t4 = qx*x*2.0;
			const VType t5 = qy*y*2.0;
			const VType t6 = qz*z*2.0;
			const VType t7 = t4+t5+t6;
			const VType t8 = qw*x*2.0;
			const VType t9 = qy*z*2.0;
			const VType t15 = qz*y*2.0;
			const VType t10 = t8+t9-t15;
			const VType t11 = qx*z*2.0;
			const VType t12 = qz*x*2.0;
			const VType t13 = qw*y*2.0;
			const VType t14 = -t11+t12+t13;
			const VType t16 = qy*x*2.0;
			J( i*3+0, 0 ) = 1.0;
			J( i*3+0, 3 ) = t7;
			J( i*3+0, 4 ) = t2+t3-qy*x*2.0;
			J( i*3+0, 5 ) = t11-qz*x*2.0-qw*y*2.0;
			J( i*3+0, 6 ) = t10;
			J( i*3+1, 1 ) = 1.0;
			J( i*3+1, 3 ) = -t2-t3+t16;
			J( i*3+1, 4 ) = t7;
