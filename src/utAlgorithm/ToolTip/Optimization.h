...
public:
	...
	void evaluate( VT1& result, const VT2& input ) const
	{
		...
		for ( InputIterator it ( m_iBegin ); it != m_iEnd; ++i, ++it )
		{
			...
			result[ i ] = static_cast< typename VT1::value_type > ( Ubitrack::Math::norm_2( vec ) );
		}
		// std::cout << "Result : \n" << result << std::endl;
	}
	...
	void jacobian( const VT2& input, MT& J ) const
	{
		...
		for ( InputIterator it ( m_iBegin ); it != m_iEnd; ++i, ++it )
		{
			...
			J( i, 0 ) = static_cast< typename MT::value_type >( t17*t42*t43 );
			J( i, 1 ) = static_cast< typename MT::value_type >( t18*t42*t44 );
			J( i, 2 ) = static_cast< typename MT::value_type >( t23*t42*t45 );
			J( i, 3 ) = static_cast< typename MT::value_type >( t42*(t6*t17*t43*2-t18*t26*t44*2+t23*t35*t45*2)*(1.0/2.0) );
			J( i, 4 ) = static_cast< typename MT::value_type >( t42*(t9*t17*t43*2+t18*t24*t44*2-t23*t37*t45*2)*(1.0/2.0) );
			J( i, 5 ) = static_cast< typename MT::value_type >( t42*(t13*t17*t43*-2+t18*t28*t44*2+t23*t33*t45*2)*(1.0/2.0) );
		}
		// std::cout << "input  :\n" << input << "\n";
