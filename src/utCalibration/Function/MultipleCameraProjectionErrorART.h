...
public:
	 ...
	 */
	MultipleCameraProjectionErrorART( const std::vector< Math::Vector< 3, VType > >& p3D, 
		const std::vector< Math::Matrix< 3, 4, VType > >& cameras, 
		const std::vector< std::pair< unsigned, unsigned > > visibilities,
		const Math::Vector< 3, VType >& centerOfGravity = boost::numeric::ublas::zero_vector< VType >( 3 )	)
		: m_p3D( p3D )
		, m_cam( cameras )
		, m_vis( visibilities )
...
protected:
	...
	const Math::Vector< 3, VType > m_centerOfGravity;
};
